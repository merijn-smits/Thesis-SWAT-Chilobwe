...
AQU_processing <- function(scenario_paths,aqu_con){
  
  #load the scenario from file
  scenario <- load_swat_run(scenario_paths, add_parameter = FALSE)
  ...
  aqu_con_file <- read.table(aqu_con, skip = 2, header = FALSE, fill = TRUE)[,c(1,4)]
